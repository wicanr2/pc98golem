...
func disk(t *testing.T, which string) *Volume {
	t.Helper()
	dir := os.Getenv("PC98_DISK_DIR")
	if dir == "" {
		t.Skip("沒有 PC98_DISK_DIR：原版映像不進版控")
	}
	matches, err := filepath.Glob(filepath.Join(dir, "*"+which+"*.fdd"))
	if err != nil || len(matches) == 0 {
		t.Skipf("在 %s 找不到 %s 的映像", dir, which)
	}
	image, err := vfd.Open(matches[0])
	if err != nil {
		t.Fatal(err)
	}
...
func TestDiskOneFilesHaveTheRightHeaders(t *testing.T) {
	volume := disk(t, "Disk 1")
...
func TestFileSizesMatchTheDirectory(t *testing.T) {
	volume := disk(t, "Disk 1")
...
// 補零之後「壞掉」看起來會和「好的」一樣。
func TestDamagedFileRefusesToReadInsteadOfZeroFilling(t *testing.T) {
	volume := disk(t, "Disk 1")
	var damaged []Entry
	for _, entry := range volume.List() {
		if entry.Damaged {
	...
	for _, entry := range damaged {
		...
		if !errors.Is(err, vfd.ErrAbsent) {
			t.Errorf("%s 的錯誤沒有包住 vfd.ErrAbsent：%v", entry.Name, err)
		}
	}
...
func TestDiskTwoFallsBackToAssumedGeometry(t *testing.T) {
	volume := disk(t, "Disk 2")
...
func TestDiskOneGeometryComesFromTheBootSector(t *testing.T) {
	volume := disk(t, "Disk 1")
